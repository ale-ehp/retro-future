...
Una scena 3D è fluida se ogni fotogramma sta dentro un budget di tempo: sedici millesimi di secondo per averne sessanta al secondo. Questo capitolo dice dove va quel tempo, che cosa è stato misurato davvero e che cosa invece resta un obiettivo.

:::

## Cosa vede l'utente

Su un computer la città scorre; su un telefono l'immagine è un po' più
morbida, perché si disegna a risoluzione più bassa con un anti-aliasing che
sfuma i bordi. Prima di partire la
...
riassunto dell'OSD. Quando il benchmark parte da un indirizzo, il file si scarica da solo come
`retro-benchmark-<data ISO>.json`, salvo chiedere il contrario; su iPhone può
aprirsi il foglio di condivisione invece del download.
...
Le cache hanno un tetto, e quelle che tengono risorse della scheda grafica le
smontano quando le espellono. I fumetti: LRU (esce la voce
...
97 moduli in `src/` per 1.428.704 byte; compressi con brotli al livello 11,
che è il comando con cui ho misurato io (Cloudflare comprime in brotli, ma il
livello che usa non è dichiarato), la somma per file è 317.479 byte, e `main.js` passa da 47.778 a 11.909.
