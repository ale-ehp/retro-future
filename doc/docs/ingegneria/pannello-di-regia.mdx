...
costruivo la demo, invece, era la cosa che guardavo di più: dieci schede, oltre
duecento cursori, venti menù a tendina e sette bottoni, che cambiano la scena mentre è
in movimento. Larghezza del boulevard, altezza dei palazzi, colore e intensità di ogni
...
- **Il pannello è ritirato, ma i valori restano.** In produzione spariscono il bottone
  ...
  che il resto del codice si aspetta. Togliere anche quelli vorrebbe dire riscrivere
  la lettura di 537 valori per un guadagno nullo.
...
- **Il servizio di salvataggio risponde solo da locale.** È uno strumento per chi
  costruisce, con un server di appoggio in ascolto; in produzione il ramo restava
...
- **Una riapplicazione per fotogramma.** Trascinare un cursore genera decine di eventi
  al secondo, e riapplicare sei domini a ognuno significa perdere il fotogramma proprio
  mentre stai giudicando l'effetto.
...
- **Il parametro che nasconde il pannello è applicato da uno script nel markup**, non
  dal modulo principale: il pannello è nel markup e il modulo arriva dopo, quindi si
  vedrebbe comparire e sparire.
...
- Tredici chiavi del file canonico configuravano uno strumento di posizionamento che
  non esiste più, e restavano inerti perché la funzione che applica salta le chiavi
  senza controllo. Le ho tolte; diciotto chiavi di un'altra famiglia restano, e una
  prova le fissa, così se cambiano è deliberato.
