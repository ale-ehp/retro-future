...
537 `getElementById` in una funzione sola, 4 aiutanti tipizzati, 12 gruppi per la via veloce e una riapplicazione per frame agganciata a `requestAnimationFrame`. Tre sorgenti di valori che si sovrappongono al boot, con il JSON canonico che vince e i clamp che possono silenziare una modifica.
