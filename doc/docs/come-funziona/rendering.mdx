...
valori lineari, prima della conversione per lo schermo. Farle dopo dà un'immagine
...
ogni fotogramma. Durante il rivelo della città, su telefono, il bloom viene saltato
del tutto finché il drone non è atterrato.
...
Il TAA esiste, con jitter di Halton su 8 campioni, clamp del vicinato e history a
16 bit, ma si accende solo con `?taa=1`.
...
Il pixel ratio massimo è 2, con un budget per lo schermo fisico: 1280x720 sotto il
Full HD, 1920x1080 sopra. Ogni 500 millisecondi il ciclo misura i fotogrammi al
...
budget automatico esiste, è provato, e non interviene. Su telefono il pixel ratio è
...
- **1,7 e non 2 su telefono.** Da 1 sono passato a 2 fisso, poi a 1,8 (19% di pixel
  in meno), poi a 1,7 (28% in meno). Decisioni mie, guardando il telefono, con FXAA
  che nasconde la leggera morbidezza.
...
- Nel ciclo dei fotogrammi non c'è un tetto agli fps: il ciclo si riprenota al
  refresh dello schermo. Il “cap” che avevo in mente è il budget a 60 di cui sopra.
...
| upscale FSR-like | scala interna 0,65..1, nitidezza 0..1,25, 11 letture | `src/engine/shaders.js` |
