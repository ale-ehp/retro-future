...
97 moduli ESM serviti senza bundler, risolti da una importmap. `main.js` monta i sottosistemi in un ordine che una prova verifica sul sorgente; lo stato condiviso è un oggetto mutabile per dominio, e chi costruisce lo fa dentro `initX(deps)` perché gli import girano prima di scena, camera e renderer.
...
pagina. Il browser carica i 97 moduli ESM così come stanno su disco, senza minificazione, uno
per richiesta.
...
- **La seconda visita costa richieste, non byte.** I moduli sono serviti con
  `cache-control: no-cache` più ETag: 97 moduli, 97 richieste condizionate per visita.
...
`controls/`, `engine/`, `world/`. Nessun file supera le 1.500 righe; il più grande,
`character/runner-crowd-runtime.js`, ne ha 1.352. Sulla rete viaggiano in brotli da
...
|---|---|---|
| moduli sotto `src/` | 97 (più 35 file di test e 1 `.d.ts`) | `find`, misurato il 2026-09-20; `README.md` |
