...
Il cielo non è una fotografia: è calcolato, nuvola per nuvola, da un programma che gira sulla scheda grafica. Calcolarlo a ogni fotogramma costerebbe metà del tempo disponibile, e questo capitolo spiega il compromesso che lo rende sostenibile.
...
Nella scena non c'è nebbia: nessun effetto di foschia è dichiarato, e i materiali la
...
`frustumCulled = false`, cioè senza il salto che di norma esclude gli oggetti fuori
dal cono della camera. A ogni fotogramma `update(now)` copia la posizione della
camera sulla sfera, così il cielo sembra all'infinito. Il vertex shader passa solo la direzione normalizzata del vertice.
...
Nel progetto non esiste `scene.fog`, e i materiali portano `fog: false`. Quello che
riempie l'aria sono le particelle atmosferiche, descritte qui sopra; nessuna fonte
dice che cosa, esattamente, produca l'impressione di foschia che resta guardando in
fondo al viale.
