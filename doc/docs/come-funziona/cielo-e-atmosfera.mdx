...
Cupola procedurale in GLSL con rumore multi-ottava, troppo cara per frame: viene cotta in un WebGLCubeRenderTarget, una faccia ogni 2 frame, e usata come `scene.background`. Da qui il moto a scatti delle nuvole e il bake a 512 su desktop, 128 su mobile.
...
Nell'aria salgono e derivano 400 punti ciano, polvere o braci, che il bloom
ammorbidisce. Nella scena non c'è nebbia.

![Il cielo procedurale sopra il boulevard](/img/cielo.webp)

## Il problema

Il cielo non è una texture: è calcolato per pixel, ogni frame, da uno shader
procedurale. Nel ramo `full`, quello di serie su desktop, ogni pixel di cielo valuta
circa 11 campi di rumore 3D a più ottave, circa 340 operazioni fra seni e hash. Il
cielo copre circa metà del frame a livello strada. Metà del frame, ogni frame, per
sempre.

Il ramo `balanced` costa circa 3,5 volte meno. Su mobile il cielo resta comunque il
primo costo di fill: spegnendolo con `?fx.sky=0` ho misurato 5,4 fps in più. Ridurre
le ottave non ha pagato: il resto dello shader gira lo stesso.

C'è un secondo problema, meno visibile: nuvole e luci vanno a tempo di
`performance.now()`, e una scena “ferma” non è mai ferma. 2 frame a 3 secondi di
distanza differiscono del 2,3%, e la baseline della mattina batteva quelle del
pomeriggio del 3,7% a codice identico. Bloccare quell'orologio non si può senza
toccare molti moduli: il README della baseline ne conta 15.
...
`frustumCulled = false`. A ogni frame `update(now)` copia la posizione della camera
sulla sfera, così il cielo sembra all'infinito. Il vertex shader passa solo la
direzione normalizzata del vertice.

### Lo shader delle nuvole

Il colore parte da un gradiente scuro fra `low` e `top`. Poi entra il campo delle
nuvole, che è rumore: hash a seno, value noise 2D e 3D, `fbm` a 4 ottave e `ridge` a
3 (`1 - |2n - 1|`). Varianti a 2 ottave servono i livelli economici su mobile.

Il punto di campionamento è `cloudP = dir * 1,45 + deriva`. Un domain warp a 3
componenti lo deforma con altro rumore prima di campionare, e da `warpedP` escono 4
campi: `broad`, `detail`, `rolling`, `under`. Il ramo `full` (`uSkyQuality > 0,5`)
spende 3 `fbm3` di warp, 2 più 2 `fbm3`/`ridge3` per `broad` e `detail`, 4 `fbm3` per
`rolling` e `under`, con un morphing fra coppie; `aggressive` e `conservative`
(`uSkyCheap` sopra 1,5 o 0,5) usano le varianti a 2 ottave; `balanced` 1 `fbm3`, 1
`ridge3` e 2 `noise3`, 7 ottave in tutto.
...
`skyDisplayColor`, finisce in `scene.background`, nella uniform `uSkyTint` e nella
forza della tinta: 0,08 per `void`, 0,26 per `steel`, 0,42 per gli altri. La qualità
viene da `?skyQuality=` se c'è, altrimenti è `balanced` su mobile, altrimenti dal
pannello. `uSkyCheap` viene da `?skyCheap=` o da `SKY_CHEAP_MOBILE_DEFAULT = 0`. I
6 cursori del temporale scrivono direttamente le uniform `uStorm*`.

### I lampi

Si accendono solo con `uLightningMode > 0,5`, la scelta di cielo del JSON canonico;
con le altre 5 (`storm`, `void`, `grid`, `teal`, `steel`) restano spenti. Un
`lightningThread` (ridge a scala 7,8 nel `full`, `noise3` nel `balanced`) è confinato
in una fascia regolata da `uStormBand` e da un `cloudGate` sulla soglia
`uStormCloudThreshold`. Sopra corrono 2 orologi: le raffiche `glitchBurst` a celle
...
Il fatto centrale: il cielo che vedete non è la cupola, è una fotografia della
cupola. Una `CubeCamera(1, 20000)` la fotografa in un `WebGLCubeRenderTarget`,
6 facce che coprono tutte le direzioni, e quel cubo diventa `scene.background`, che
three.js campiona a buon mercato. La cupola vera, `domeMesh`, si nasconde.

Il bake è acceso di default su desktop e mobile, con cubo di 512 pixel per faccia sul
primo e 128 sul secondo; `?skyBake=0|1` forza. È “spread”: rifaccio una faccia
ogni `SKY_BAKE_FACE_STRIDE = round(12 / 6) = 2` frame, e il ciclo delle 6 facce dura
12 frame. `?skyBakeSpread=0` torna al bake intero ogni 12.
...
Le 6 facce di un ciclo condividono lo stesso istante, `cycleNow`, catturato sulla
faccia 0. Ogni faccia usa la camera figlia `cubeCam.children[faceIndex]` e
`renderer.setRenderTarget(cubeRT, faceIndex, 0)`; le mip si generano solo
sull'ultima, si alza `needsPMREMUpdate`, e si ripristinano target, faccia attiva, mip
e `xr.enabled`. La sorgente è la scena cielo del rivelo con `background = null`: la
cupola copre tutte le direzioni, una tinta piatta di clear non serve.

Il bake vale solo in stato stazionario:
`syncSkyBackgroundBake(now, cityRevealComplete && !isCityRevealCompositeActive())`.
Fuori di lì si azzera e la cupola torna visibile con
`scene.background = skyDisplayColor`. Se il profilo cambia a metà sessione, il cubo
si ricostruisce alla dimensione giusta.

Una conseguenza: la cupola nascosta non avanza `uTime`, che `update(now)` muove solo
quando è visibile; il tempo del cielo procede ai bake, a scatti di 12 frame.
...
a 0. Quella scena è il primo pass dell'EffectComposer (`RenderPass` con `clear = true`)
o il primo render diretto.
...
Il fragment shader disegna un disco morbido, scarta fuori dal raggio e somma in
additivo con `depthWrite: false`, `renderOrder = 22`, `boundingSphere` di raggio 1e9 e
`frustumCulled = false`. L'orologio è `uTime = now * 0,001`, lo stesso degli impulsi
sui LED. Si vedono solo a rivelo completato.

### La nebbia che non c'è

Nel progetto non esiste `scene.fog`, e i materiali portano `fog: false`. A riempire
l'aria sono le particelle atmosferiche; nessuna fonte dice che cosa produca
l'impressione di foschia che resta guardando in fondo al viale.
...
Per il gate visivo conta: la fotografia si scatta a `performance.now() >= 60000`, così
le fasi di luci e nuvole coincidono fra sessioni, con una raffica di 40 frame. A
istante fisso 2 sessioni differiscono dello 0,22%.

### Interruttori e ispezione

`?fx.sky=0` e `?fx.particles=0` spengono i 2 sottosistemi, solo sul profilo mobile
o con `?forceMobile=1`. Dalla console `window.__tronInspect()` espone `.storm` (le
...
- **Il bake, non la riduzione delle ottave.** Ridurre le ottave non pagava, il resto
  dello shader gira comunque; meglio fotografare il cielo in un cubo da 128 pixel ogni
  12 frame e riusarlo come `scene.background`. Vale perché il cielo è uno sfondo
  liscio all'infinito: il cubo resta valido mentre la camera si muove, e solo la
  deriva lenta si aggiorna al bake successivo.
- **Partito spento, promosso dopo un A/B sul dispositivo.** Il default mobile è
  rimasto `false` finché il confronto non ha confermato che il guadagno di fill
  batteva il picco periodico del bake.
- **Acceso anche su desktop, con cubo da 512.** Il ramo `full` è il default lì e
  valuta circa 11 campi di rumore per pixel; il cubo più grande tiene il dettaglio per
  cui quel ramo esiste.
- **Un istante solo per le 6 facce.** Senza `cycleNow` ogni faccia era un paio di
  frame più avanti della precedente: cuciture lungo gli spigoli del cubo.
...
- **Mobile forzato a `balanced`.** Il default `full` del pannello passava da
  `applySkyPreset` senza un cancello per il mobile, e gli screenshot `full` e
...
- **Il gate fotografa a 60 secondi di orologio.** Le fasi di luci e nuvole coincidono
  fra sessioni; la raffica di 40 frame fa il resto.
...
- Con il bake attivo le nuvole avanzano a scatti di 12 frame; nessun documento
  dice se è stato accettato o misurato.
- Il README della baseline conta 15 file con `performance.now()`; il conteggio
...
| cubo del bake | 512 desktop, 128 mobile; ciclo 12 frame; una faccia ogni 2 | `src/world/sky-dome.js`; `src/world/sky-dome.test.mjs` |
...
| cielo spento su mobile | +5,4 fps | commit 78247e3f, 2026-07-09 |
...
| deriva fra frame | 2,3% a 3 s, 0,4% a 11 s, 0,22% a istante fisso, 3,7% mattina contro pomeriggio | `test/retro-future-baseline/README.md`, misure del 2026-09-19 |
...
I test: `sky-dome.test.mjs` prima leggeva il sorgente, ora costruisce la cupola,
applica la luminosità e verifica che un rebake completo si distribuisca sulla finestra
di stride originale. `src/invarianti.test.mjs` fissa l'ordine di disegno (i
...
Il bake del cielo è una cache con scadenza di 12 frame, aggiornata a rotazione
(una faccia ogni 2) invece che tutta insieme, e `cycleNow` è la lettura coerente: le
6 facce vedono lo stesso istante, come una transazione su uno snapshot.
`scene.background` è la risposta servita dalla cache; la cupola nascosta è l'origin
