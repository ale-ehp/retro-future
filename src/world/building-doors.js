...
function createSideBuildingCivicNumberTexture(value) {
  ...
  ctx.lineWidth = 44;
  ctx.strokeText(text, x, y);
  ctx.shadowColor = SIDE_BUILDING_CIVIC_NUMBER_SHADOW_STYLE;
  ctx.shadowBlur = 18;
  ctx.strokeStyle = SIDE_BUILDING_CIVIC_NUMBER_BRIGHT_STROKE_STYLE;
  ctx.lineWidth = 34;
  ctx.strokeText(text, x, y);
  ctx.shadowBlur = 8;
  ctx.strokeStyle = SIDE_BUILDING_CIVIC_NUMBER_HIGHLIGHT_STROKE_STYLE;
  ctx.lineWidth = 12;
  ctx.strokeText(text, x, y);
  ctx.shadowBlur = 0;
  ctx.fillStyle = SIDE_BUILDING_CIVIC_NUMBER_FILL_STYLE;
