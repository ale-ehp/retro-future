...
const TRON_FSR_UPSCALE_SHADER = {
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      vec3 color = tronRcasLikeSharpen(reconstructed, north, south, east, west, rcasAmount);
...
const TRON_SYNTH_MUSIC_ENABLED = true;
const TRON_SYNTH_MUSIC_BPM = 96;
const TRON_SYNTH_MUSIC_MASTER_GAIN = 0.14;
const TRON_SYNTH_MUSIC_LOOKAHEAD_MS = 45;
const TRON_SYNTH_MUSIC_SCHEDULE_AHEAD = 0.32;
const TRON_SYNTH_MUSIC_STEP_SEC = 60 / TRON_SYNTH_MUSIC_BPM / 4;
const TRON_SYNTH_MUSIC_PATTERN_STEPS = 64;
const TRON_SOUNDTRACK_ENABLED = true;
const TRON_SOUNDTRACK_URL = 'audio/music/retro-future.opus';
const TRON_SOUNDTRACK_VOLUME = 0.09;
const TRON_SOUNDTRACK_FADE_IN_SECONDS = 3;
const TRON_SOUNDTRACK_STOP_FADE_SECONDS = 0.75;
const TRON_SOUNDTRACK_CROSSFADE_SECONDS = 8;
const TRON_SOUNDTRACK_INITIAL_START_SECONDS = 0;
const TRON_SOUNDTRACK_LOOP_START_SECONDS = 30;
const TRON_SOUNDTRACK_INTRO_FX_FADE_SECONDS = 0.12;
const TRON_SOUNDTRACK_INTRO_FX_REVEAL_STOP_DELAY_MS = 350;
const TRON_SOUNDTRACK_INTRO_FX_CURVE_SIZE = 4096;
const TRON_SOUNDTRACK_INTRO_FX_WOBBLE_RATE_HZ = 7.5;
const TRON_SOUNDTRACK_INTRO_FX_MAX_WOBBLE_DEPTH_HZ = 1600;
const TRON_SOUNDTRACK_INTRO_FX_MAX_NOISE_GAIN = 0.055;
const TRON_SOUNDTRACK_INTRO_FX_DEFAULTS = Object.freeze({
  enabled: true,
  mix: 1,
  crusher: 0,
  bitDepth: 8,
...
function createTronSoundtrackElement() {
  ...
  audio.addEventListener('loadedmetadata', () => {
    if (Number.isFinite(audio.duration)) tronSoundtrack.duration = audio.duration;
  });
  audio.addEventListener('error', () => {
    const code = audio.error?.code || 0;
    tronSoundtrack.error = `Soundtrack load error ${code}`;
    console.warn('[tron-soundtrack]', tronSoundtrack.error);
  });
  return audio;
}
...
function setupTronSoundtrackGraph(ctx) {
  if (tronSoundtrack.ready) return true;
  tronSoundtrack.elements = [createTronSoundtrackElement(), createTronSoundtrackElement()];
  tronSoundtrack.sources = [];
  tronSoundtrack.gains = [];
  tronSoundtrack.dryGains = [];
  tronSoundtrack.introLofiGains = [];
  tronSoundtrack.introHighpassFilters = [];
  tronSoundtrack.introLowpassFilters = [];
  tronSoundtrack.introLofiShapers = [];
  tronSoundtrack.introDistortionShapers = [];
  tronSoundtrack.introLofiLfos = [];
  tronSoundtrack.introLofiLfoGains = [];
  tronSoundtrack.elements.forEach((audio) => {
    ...
    introLofiShaper.curve = createTronIntroBitcrushCurve(tronSoundtrack.introFx.bitDepth, tronSoundtrack.introFx.crusher);
    introLofiShaper.oversample = 'none';
    introDistortionShaper.curve = createTronIntroDistortionCurve(tronSoundtrack.introFx.distortion);
    ...
    tronSoundtrack.sources.push(source);
    tronSoundtrack.gains.push(gain);
    tronSoundtrack.dryGains.push(dryGain);
    tronSoundtrack.introLofiGains.push(introLofiGain);
    tronSoundtrack.introHighpassFilters.push(introHighpassFilter);
    tronSoundtrack.introLowpassFilters.push(introLowpassFilter);
    tronSoundtrack.introLofiShapers.push(introLofiShaper);
    tronSoundtrack.introDistortionShapers.push(introDistortionShaper);
    tronSoundtrack.introLofiLfos.push(introLofiLfo);
    tronSoundtrack.introLofiLfoGains.push(introLofiLfoGain);
    audio.load();
  });
  ...
  noiseSource.start();
  tronSoundtrack.introNoiseSource = noiseSource;
  tronSoundtrack.introNoiseFilter = noiseFilter;
  tronSoundtrack.introNoiseGain = noiseGain;
  tronSoundtrack.ready = true;
  applyTronSoundtrackIntroLofiMix(tronSoundtrack.introLofiActive, 0.01);
  return true;
}

function tronSoundtrackLoopStart() {
  const duration = tronSoundtrack.duration || tronSoundtrack.elements[tronSoundtrack.activeIndex]?.duration || 0;
...
function pauseTronSoundtrackElement(index) {
  const audio = tronSoundtrack.elements[index];
  if (!audio) return;
  audio.pause();
  setAudioCurrentTime(audio, tronSoundtrackLoopStart());
}

function startTronSoundtrackElement(index, startAt, fadeSeconds, volume = tronSoundtrack.targetVolume) {
  const audio = tronSoundtrack.elements[index];
  const gain = tronSoundtrack.gains[index];
  if (!audio || !gain) return false;
  const duration = Number.isFinite(audio.duration) ? audio.duration : tronSoundtrack.duration;
  ...
  gain.gain.cancelScheduledValues(footstepAudioContext.currentTime);
  gain.gain.setValueAtTime(0.0001, footstepAudioContext.currentTime);
  const playPromise = audio.play();
  if (playPromise?.catch) {
    playPromise.catch((error) => {
      tronSoundtrack.error = error?.message || String(error);
      console.warn('[tron-soundtrack]', tronSoundtrack.error);
    });
  }
  rampGain(gain, volume, fadeSeconds, 0.0001);
  tronSoundtrack.lastStartAt = safeStart;
  return true;
}

function crossfadeTronSoundtrack() {
  if (!tronSoundtrack.playing || tronSoundtrack.crossfading || !footstepAudioContext) return false;
  const activeIndex = tronSoundtrack.activeIndex;
  const nextIndex = activeIndex === 0 ? 1 : 0;
  const activeGain = tronSoundtrack.gains[activeIndex];
  const loopStart = tronSoundtrackLoopStart();
  tronSoundtrack.crossfading = true;
  startTronSoundtrackElement(nextIndex, loopStart, TRON_SOUNDTRACK_CROSSFADE_SECONDS, tronSoundtrack.targetVolume);
  rampGain(activeGain, 0.0001, TRON_SOUNDTRACK_CROSSFADE_SECONDS);
  window.setTimeout(() => {
    pauseTronSoundtrackElement(activeIndex);
    tronSoundtrack.activeIndex = nextIndex;
    tronSoundtrack.crossfading = false;
    tronSoundtrack.loopCount += 1;
    tronSoundtrack.lastLoopAt = performance.now();
  }, Math.ceil(TRON_SOUNDTRACK_CROSSFADE_SECONDS * 1000) + 80);
  return true;
}

function monitorTronSoundtrackLoop() {
  if (!tronSoundtrack.playing || tronSoundtrack.crossfading) return;
  const active = tronSoundtrack.elements[tronSoundtrack.activeIndex];
  const duration = Number.isFinite(active?.duration) ? active.duration : tronSoundtrack.duration;
  if (!active || !Number.isFinite(duration) || duration <= TRON_SOUNDTRACK_CROSSFADE_SECONDS + 2) return;
  tronSoundtrack.duration = duration;
...
function startTronFileSoundtrack(source = 'manual') {
  if (!TRON_SOUNDTRACK_ENABLED || tronSoundtrack.playing) return Boolean(tronSoundtrack.playing);
  const ctx = ensureTronAudioContext();
  if (!ctx) {
    tronSoundtrack.error = 'AudioContext unavailable';
    return false;
  }
  setupTronSoundtrackGraph(ctx);
  tronSoundtrack.playing = true;
  tronSoundtrack.startedAt = performance.now();
  tronSoundtrack.startSource = source;
  tronSoundtrack.activeIndex = 0;
  tronSoundtrack.crossfading = false;
...
function stopTronFileSoundtrack(fadeSeconds = TRON_SOUNDTRACK_STOP_FADE_SECONDS) {
  if (!tronSoundtrack.playing) return false;
  if (tronSoundtrack.timer) {
    window.clearInterval(tronSoundtrack.timer);
    tronSoundtrack.timer = 0;
  }
  tronSoundtrack.playing = false;
  tronSoundtrack.crossfading = false;
  setTronSoundtrackIntroLofi(false, fadeSeconds);
  tronSoundtrack.gains.forEach((gain) => rampGain(gain, 0.0001, fadeSeconds));
  window.setTimeout(() => {
    tronSoundtrack.elements.forEach((audio) => {
      audio.pause();
...
function setTronFileSoundtrackVolume(value = TRON_SOUNDTRACK_VOLUME) {
  ...
  tronSoundtrack.targetVolume = next;
  if (!tronSoundtrack.playing) return true;
  const activeGain = tronSoundtrack.gains[tronSoundtrack.activeIndex];
  const inactiveGain = tronSoundtrack.gains[tronSoundtrack.activeIndex === 0 ? 1 : 0];
  rampGain(activeGain, next, 0.08);
  if (!tronSoundtrack.crossfading) rampGain(inactiveGain, 0.0001, 0.08);
  return true;
}
...
function readBridgeNumber(record, key) {
...
function readBridgeVisible(record) {
  const input = getBridgeControl(record.index, 'visible');
  return input ? input.checked : true;
}

function updateBridgeControlOutputs() {
...
function applyBridgeFixedDefaults() {
...
const CITY_DEPARTMENT_BOARD_ENABLED = true;
...
const cityDepartmentBoardPanelMat = new THREE.MeshStandardMaterial({
  color: 0x001216,
  metalness: 0.45,
  roughness: 0.18,
  envMap: reflectionEnvMap,
...
function drawCityDepartmentBoardTexture(board, now = performance.now()) {
  ...
  const switchProgress = state.switching
    ...
    : 0;
  const oldItems = cityDepartmentPageItems(state.page);
  const newItems = cityDepartmentPageItems(state.targetPage);
  ...
  for (let row = 0; row < CITY_DEPARTMENT_BOARD_ROWS; row++) {
    ...
    const label = state.switching
      ? cityDepartmentInterpolatedText(oldLabel, newLabel, row, switchProgress, state.textureUpdates)
      : oldLabel;
...
function addCityDepartmentFrame(group, width, height, z = 0.06) {
  const hw = width / 2;
  const hh = height / 2;
  const thick = 0.18;
...
function cityDepartmentBoardBottomY() {
  return Number.isFinite(droneLandingPose?.y) ? droneLandingPose.y : DEFAULT_DRONE_LANDING_POSE.y;
}
...
function syncCityDepartmentBoardPerimeterPose(board) {
  ...
  if (pad?.border?.position && polygon?.length) {
    ...
    const playerSideSign = (playerSpawn?.z ?? camera.position.z ?? record.mesh.position.z) >= record.mesh.position.z ? 1 : -1;
    const centerX = (minX + maxX) * 0.5;
...
function cityDepartmentBoardPoseSignature() {
  const parts = [
    ...
    (playerSpawn?.z ?? camera.position.z ?? 0).toFixed(3),
  ];
...
function updateCityDepartmentBoards(now) {
...
function cityDepartmentBoardRevealFactor() {
  if (!CITY_DEPARTMENT_BOARD_REVEAL_WITH_RUNNERS || !TRON_RUNNER_REVEAL_ENABLED) return 1;
  if (!cityRevealComplete || !tronRunnerState.ready) return 0;
  if (tronRunnerRevealComplete) return 1;
  if (!tronRunnerRevealStartedAt && !tronRunnerRevealActive) return 0;
  return THREE.MathUtils.clamp(tronRunnerRevealProgress, 0, 1);
}
...
function syncCityDepartmentBoardRevealVisibility(revealFactor = cityDepartmentBoardRevealFactor()) {
  ...
  const visible = Boolean(CITY_DEPARTMENT_BOARD_ENABLED && (factor > 0.002 || tronRunnerRevealActive));
  if (
...
function cityDepartmentBoardInspect() {
  ...
  return {
    ...
    items: cityDepartmentPageItems(firstState?.page ?? 0),
...
const CITY_ROLE_BOARD_ENABLED = true;
...
function syncCityRoleBoardDoorPose(board) {
...
// ---------- Real demo: phosphor audio equalizer on the start-side wall ----------
const LAB_EQUALIZER_ENABLED = true;
const LAB_EQUALIZER_AUDIO_URL = TRON_SOUNDTRACK_URL;
const LAB_EQUALIZER_BAR_COUNT = 16;
const LAB_EQUALIZER_SEGMENT_ROWS = 22;
const LAB_EQUALIZER_BOARD_WIDTH = 122;
const LAB_EQUALIZER_BOARD_HEIGHT = 58;
const LAB_EQUALIZER_SCENE_SCALE = 1;
const LAB_EQUALIZER_WORLD_WIDTH = LAB_EQUALIZER_BOARD_WIDTH * LAB_EQUALIZER_SCENE_SCALE;
const LAB_EQUALIZER_WORLD_HEIGHT = LAB_EQUALIZER_BOARD_HEIGHT * LAB_EQUALIZER_SCENE_SCALE;
const LAB_EQUALIZER_CANVAS_WIDTH = 768;
const LAB_EQUALIZER_CANVAS_HEIGHT = 384;
const LAB_EQUALIZER_GROUP_RENDER_ORDER = 18;
const LAB_EQUALIZER_PANEL_RENDER_ORDER = 18;
const LAB_EQUALIZER_DISPLAY_RENDER_ORDER = 19;
const LAB_EQUALIZER_TEXTURE_INTERVAL_MS = 1000 / 20;
const LAB_EQUALIZER_TEXTURE_ADAPTIVE_SOFT_INTERVAL_MS = 1000 / 5;
const LAB_EQUALIZER_TEXTURE_ADAPTIVE_HARD_INTERVAL_MS = 1000 / 4;
const LAB_EQUALIZER_AUDIO_SAMPLE_INTERVAL_MS = 1000 / 30;
const LAB_EQUALIZER_OFFSCREEN_SAMPLE_INTERVAL_MS = 1000;
const LAB_EQUALIZER_AUDIO_KICK_SAMPLE_INTERVAL_MS = 1000 / 8;
const LAB_EQUALIZER_IDLE_SAMPLE_INTERVAL_MS = 250;
const LAB_EQUALIZER_OFFSCREEN_TEXTURE_INTERVAL_MS = 1000 / 4;
const LAB_EQUALIZER_VISIBILITY_INTERVAL_MS = 1000 / 12;
const LAB_EQUALIZER_POSE_INTERVAL_MS = 250;
const LAB_EQUALIZER_GRAPH_INTERVAL_MS = 500;
const LAB_EQUALIZER_STATIC_TEXTURE_REFRESH_MS = 500;
const LAB_EQUALIZER_START_WALL_OFFSET_Z = GRID_BLOCK * 3;
const LAB_EQUALIZER_START_WALL_INSET_Y = 6;
const LAB_EQUALIZER_VERTICAL_OFFSET = 0;
const LAB_EQUALIZER_MIN_HZ = 35;
const LAB_EQUALIZER_MAX_HZ = 15500;
const LAB_EQUALIZER_FFT_SIZE = 4096;
const LAB_EQUALIZER_ANALYSER_MIN_DB = -96;
const LAB_EQUALIZER_ANALYSER_MAX_DB = -6;
const LAB_EQUALIZER_ANALYSER_SMOOTHING = 0.28;
const LAB_EQUALIZER_LEVEL_DB_FLOOR = -88;
const LAB_EQUALIZER_LEVEL_DB_CEILING = -18;
const LAB_EQUALIZER_RELATIVE_DB_RANGE = 42;
const LAB_EQUALIZER_LEVEL_GAMMA = 1.55;
const LAB_EQUALIZER_PEAK_HOLD_SECONDS = 0.14;
const LAB_EQUALIZER_PEAK_MIN_FALL_ROWS_PER_SEC = 1.8;
const LAB_EQUALIZER_PEAK_MAX_FALL_ROWS_PER_SEC = 52;
const LAB_EQUALIZER_BASS_BAND_COUNT = 4;
const LAB_EQUALIZER_VISIBILITY_RADIUS = Math.hypot(LAB_EQUALIZER_WORLD_WIDTH, LAB_EQUALIZER_WORLD_HEIGHT) * 0.55;
const labEqualizerGroup = new THREE.Group();
...
const LAB_EQUALIZER_PULSE_CONTROLS = Object.freeze([
...
function syncLabEqualizerPose() {
  const roadMaxZ = dynamicRoadCenter + dynamicRoadLength * 0.5;
  const startReferenceZ = Math.max(
    Number.isFinite(roadMaxZ) ? roadMaxZ : -Infinity,
    Number.isFinite(playerSpawn?.z) ? playerSpawn.z : -Infinity,
    Number.isFinite(droneLandingPose?.z) ? droneLandingPose.z : -Infinity,
  );
...
function labEqualizerTextureIntervalMs() {
  ...
  const measuredFps = Number.isFinite(latestMeasuredFps) ? latestMeasuredFps : 0;
...
function buildLabEqualizer({ visible = cityRevealComplete } = {}) {
...
function activateLabEqualizerAfterReveal() {
  if (!LAB_EQUALIZER_ENABLED || labEqualizerActivatedAfterReveal || !cityRevealComplete) return false;
  buildLabEqualizer();
...
function updateLabEqualizer(now, dt) {
...
const TRON_RUNNER_MODEL_URL = 'character-mockups/assets/models/soldier.glb';
const TRON_RUNNER_ENABLED = true;
const TRON_RUNNER_SOURCE_CHARACTER_VISIBLE = false;
const TRON_RUNNER_TARGET_HEIGHT = 8.66;
const TRON_RUNNER_DEFAULT_SPEED = 2.8;
const TRON_RUNNER_ROUTE_OFFSET = 0.25;
const TRON_RUNNER_GROUND_OFFSET = 0.04;
const TRON_RUNNER_SIDEWALK_SIGN = -1;
const TRON_RUNNER_SIDEWALK_INSET = 7.2;
const TRON_RUNNER_FREE_ROAM_ENABLED = true;
const TRON_RUNNER_FREE_ROAM_COLLISIONS_ENABLED = true;
const TRON_RUNNER_FREE_ROAM_FOOTSTEPS_ENABLED = true;
const TRON_RUNNER_FREE_ROAM_REACH_RADIUS = 4.2;
const TRON_RUNNER_FREE_ROAM_COLLISION_RADIUS = 1.1;
const TRON_RUNNER_FREE_ROAM_SIDEWALK_INSET = 7.2;
const TRON_RUNNER_FREE_ROAM_STRIDE_LENGTH = 2.15;
const TRON_RUNNER_WALK_CYCLE_DISTANCE = TRON_RUNNER_FREE_ROAM_STRIDE_LENGTH * 1.5;
const TRON_RUNNER_DISTANCE_DRIVEN_WALK_ENABLED = true;
const TRON_RUNNER_CROWD_DISTANCE_DRIVEN_WALK_ENABLED = true;
const TRON_RUNNER_FOOTSTEP_VOLUME_SCALE = 3.264;
const TRON_RUNNER_FOOTSTEP_REF_DISTANCE = 8;
const TRON_RUNNER_FOOTSTEP_MAX_DISTANCE = 92;
const TRON_RUNNER_FOOTSTEP_ROLLOFF = 1.35;
const TRON_RUNNER_FOOTSTEP_CONTACTS = Object.freeze([
  { phase: 0.18, side: 'left' },
  { phase: 0.68, side: 'right' },
]);
const TRON_RUNNER_SUIT_TEXTURE_MODE = 'procedural-uv-suit';
const TRON_RUNNER_SUIT_COLOR = new THREE.Color(0xffffff);
const TRON_RUNNER_SUIT_EMISSIVE = new THREE.Color(0xffffff);
const TRON_RUNNER_CHARACTER_LED_BRIGHTNESS_MULTIPLIER = 1.15;
const TRON_RUNNER_CHARACTER_LED_BLOOM_BOOST = 1.77;
const TRON_RUNNER_CHARACTER_LED_EMISSIVE_MAX = 1.76;
const TRON_RUNNER_CROWD_LED_EMISSIVE_INTENSITY = 1.62;
const TRON_RUNNER_BEAT_PULSE_ENABLED = true;
const TRON_RUNNER_BEAT_PULSE_BPM = 128;
const TRON_RUNNER_BEAT_PULSE_OFFSET_SECONDS = 0;
const TRON_RUNNER_BEAT_PULSE_INTENSITY = 3;
const TRON_RUNNER_BEAT_PULSE_DECAY = 8;
const TRON_RUNNER_BEAT_PULSE_DIVISION = 1;
const TRON_RUNNER_BEAT_PULSE_MAX_MULTIPLIER = 4;
const TRON_RUNNER_BEAT_PULSE_AUDIO_KICK_ENABLED = false;
const TRON_RUNNER_BEAT_PULSE_AUDIO_SAMPLE_MAX_AGE_MS = 360;
const TRON_RUNNER_BEAT_PULSE_LOW_BAND_ENABLED = true;
const TRON_RUNNER_BEAT_PULSE_BASS_BODY_THRESHOLD = 0.32;
const TRON_RUNNER_BEAT_PULSE_BASS_BODY_GAIN = 0.45;
const TRON_RUNNER_BEAT_PULSE_MATERIAL_SKIP_ENABLED = true;
const TRON_RUNNER_BEAT_PULSE_MATERIAL_EPS = 0.0005;
const TRON_RUNNER_SHADOW_COLOR = new THREE.Color(0x000305);
const TRON_RUNNER_SHADOW_CYAN_COLOR = new THREE.Color(0x031a1f);
const TRON_RUNNER_LIGHTING_MODE = 'self-lit-material';
const TRON_RUNNER_GROUND_SHADOW_ENABLED = true;
const TRON_RUNNER_CONTACT_SHADOW_MAX_OPACITY = 0.28;
const TRON_RUNNER_REAL_SHADOW_ENABLED = false;
const TRON_RUNNER_REAL_SHADOW_LAYER = 4;
const TRON_RUNNER_REAL_SHADOW_MAP_SIZE = 1024;
const TRON_RUNNER_REAL_SHADOW_CAMERA_SIZE = 18;
const TRON_RUNNER_REAL_SHADOW_RECEIVER_RADIUS = 8.5;
const TRON_RUNNER_REAL_SHADOW_BASE_OPACITY = 0.22;
const TRON_RUNNER_REAL_SHADOW_COLOR = 0x000203;
const TRON_RUNNER_CONTACT_SHADOW_ROUNDNESS = 0.96;
const TRON_RUNNER_DYNAMIC_REFLECTION_ENABLED = true;
const TRON_RUNNER_DYNAMIC_REFLECTION_MAX_OPACITY = 0.34;
const TRON_RUNNER_DYNAMIC_REFLECTION_OPACITY_SCALE = 0.3;
const TRON_RUNNER_DYNAMIC_REFLECTION_BODY_OPACITY_MULTIPLIER = 1.55;
const TRON_RUNNER_DYNAMIC_REFLECTION_LED_OPACITY_MULTIPLIER = 1.08;
const TRON_RUNNER_DYNAMIC_REFLECTION_BODY_MAX_OPACITY = 0.28;
const TRON_RUNNER_DYNAMIC_REFLECTION_LED_MAX_OPACITY = 0.22;
const TRON_RUNNER_DYNAMIC_REFLECTION_BODY_RENDER_ORDER = 2;
const TRON_RUNNER_DYNAMIC_REFLECTION_LED_RENDER_ORDER = 3;
const TRON_RUNNER_DYNAMIC_REFLECTION_Y_SCALE = 0.58;
const TRON_RUNNER_DYNAMIC_REFLECTION_Y = 0.018;
const TRON_RUNNER_CROWD_TARGET_FPS = 45;
const TRON_RUNNER_CROWD_UPDATE_INTERVAL = 1 / TRON_RUNNER_CROWD_TARGET_FPS;
const TRON_RUNNER_CROWD_MAX_ACCUMULATED_DT = 0.12;
const TRON_RUNNER_CROWD_REFLECTION_MAX_ACTIVE = 3;
const TRON_RUNNER_CROWD_REFLECTION_NEAR_DISTANCE = 240;
const TRON_RUNNER_CROWD_REFLECTION_MIN_FPS = 50;
const TRON_RUNNER_CROWD_REFLECTION_REVEAL_ENABLED = false;
const TRON_RUNNER_CROWD_REFLECTION_POST_REVEAL_RAMP_ENABLED = true;
const TRON_RUNNER_CROWD_REFLECTION_POST_REVEAL_RAMP_MS = 2400;
const TRON_RUNNER_REVEAL_ENABLED = true;
const TRON_RUNNER_REVEAL_DURATION_MS = 1400;
const TRON_RUNNER_REVEAL_SCAN_RADIUS = 2.7;
const TRON_RUNNER_REVEAL_SCAN_TUBE = 0.07;
const TRON_RUNNER_REVEAL_EMISSIVE_BOOST = 2.3;
const TRON_RUNNER_REVEAL_SCAN_OUTER_OPACITY = 1;
const TRON_RUNNER_REVEAL_SCAN_CORE_OPACITY = 1;
const TRON_RUNNER_CROWD_ENABLED = true;
const TRON_RUNNER_CROWD_COUNT = 29;
const TRON_RUNNER_CROWD_START_CLUSTER_COUNT = 3;
const TRON_RUNNER_CROWD_SIDE_STREET_GROUP_EXTRA_COUNT = 4;
const TRON_RUNNER_CROWD_LOOP_FRONT_ONLY = true;
const TRON_RUNNER_IDLE_CHARACTER_ENABLED = true;
const TRON_RUNNER_IDLE_CHARACTER_STATIC = true;
const TRON_RUNNER_IDLE_CHARACTER_CIVIC = 2;
const TRON_RUNNER_IDLE_CHARACTER_COLOR_PRESET = 'orange';
const TRON_RUNNER_IDLE_CHARACTER_BODY_CLEARANCE = 1.1;
const TRON_RUNNER_IDLE_CHARACTER_FRONT_WALL_CLEARANCE = 1.35;
const TRON_RUNNER_IDLE_CHARACTER_WALL_CONTACT_EPS = 0.05;
const TRON_RUNNER_IDLE_CHARACTER_CORNER_FLAT_INSET = 2.4;
const TRON_RUNNER_IDLE_CHARACTER_LEAN_DEG = 5;
const TRON_RUNNER_IDLE_CHARACTER_LIFT_PX = 10;
const TRON_RUNNER_IDLE_CHARACTER_Y_LIFT = TRON_RUNNER_IDLE_CHARACTER_LIFT_PX * 0.05;
const TRON_RUNNER_CROWD_EXCLUDED_CIVICS = new Set([TRON_RUNNER_IDLE_CHARACTER_CIVIC]);
const TRON_RUNNER_CROWD_SCALE_LOCK = 0.55;
const TRON_RUNNER_CROWD_GROUND_OFFSET = TRON_RUNNER_GROUND_OFFSET;
const TRON_RUNNER_CROWD_PATH_MODE = 'sidewalk-building-loops';
const TRON_RUNNER_CROWD_COLLISIONS_ENABLED = true;
const TRON_RUNNER_CROWD_REACH_RADIUS = 0.85;
const TRON_RUNNER_CROWD_COLLISION_RADIUS = 1.1;
const TRON_RUNNER_CROWD_BUILDING_GUARD = 2.4;
const TRON_RUNNER_CROWD_SIDEWALK_LANE_EDGE_INSET = 1.2;
const TRON_RUNNER_CROWD_SIDEWALK_ROUTE_END_INSET = 10;
const TRON_RUNNER_CROWD_MAX_MOVE_SUBSTEP = 0.035;
const TRON_RUNNER_CROWD_SPEED_SCALE = 0.72;
const TRON_RUNNER_CROWD_ROUTE_RECORD_SPREAD = Math.max(1, TRON_RUNNER_CROWD_COUNT - TRON_RUNNER_CROWD_START_CLUSTER_COUNT);
const TRON_RUNNER_CROWD_INTELLIGENCE_ENABLED = true;
const TRON_RUNNER_CROWD_AVOIDANCE_ENABLED = true;
const TRON_RUNNER_CROWD_AVOIDANCE_RADIUS = 5.2;
const TRON_RUNNER_CROWD_AVOIDANCE_STRENGTH = 7.5;
const TRON_RUNNER_CROWD_SPATIAL_CELL = 9.5;
const TRON_RUNNER_CROWD_TURN_DURATION_MS = 180;
const TRON_RUNNER_CROWD_YIELD_DURATION_MS = 260;
const TRON_RUNNER_CROWD_PASSING_PUSH = 3.8;
const TRON_RUNNER_CROWD_DEADLOCK_MS = 850;
const TRON_RUNNER_CROWD_DEADLOCK_MOVE_EPS = 0.018;
const TRON_RUNNER_CROWD_DEADLOCK_NUDGE = 3.4;
const TRON_RUNNER_CROWD_LOD_NEAR_DISTANCE = 190;
const TRON_RUNNER_CROWD_LOD_MID_DISTANCE = 520;
const TRON_RUNNER_CROWD_CULLING_ENABLED = true;
const TRON_RUNNER_CROWD_CULL_DISTANCE = 900;
const TRON_RUNNER_CROWD_CULL_RADIUS = 24;
const TRON_RUNNER_CROWD_CULLED_LOD_STRIDE = 12;
const TRON_RUNNER_CROWD_DISTANCE_CACHE_ENABLED = true;
const TRON_MAIN_PLAYER_BODY_ENABLED = false;
const TRON_MAIN_PLAYER_BODY_REVEAL_WITH_CHARACTERS = true;
const TRON_MAIN_PLAYER_BODY_CAMERA_OFFSET = new THREE.Vector3(0, -0.02, -0.02);
const TRON_MAIN_PLAYER_BODY_SCALE = 1;
const TRON_MAIN_PLAYER_BODY_LED_COLOR = new THREE.Color(0x8bffff);
const TRON_MAIN_PLAYER_BODY_BLACK_COLOR = new THREE.Color(0x010406);
...
function makeTronRunnerShadowTexture(softness = 0.9) {
  const size = 128;
  const canvas = document.createElement('canvas');
  canvas.width = size;
  canvas.height = size;
...
function configureTronRunnerSuitTexture(texture) {
...
function makeTronRunnerSuitTexture() {
  const size = 512;
  const canvas = document.createElement('canvas');
  canvas.width = size;
  canvas.height = size;
  ...
  const texture = new THREE.CanvasTexture(canvas);
  texture.colorSpace = THREE.SRGBColorSpace;
  return configureTronRunnerSuitTexture(texture);
}

function makeTronRunnerSuitEmissiveTexture() {
  const size = 512;
  const canvas = document.createElement('canvas');
  canvas.width = size;
  canvas.height = size;
...
function makeTronRunnerSuitLedMaskTexture() {
  const size = 512;
  const canvas = document.createElement('canvas');
  canvas.width = size;
  canvas.height = size;
...
let tronRunnerShadowTexture = makeTronRunnerShadowTexture();
let tronRunnerShadowTextureSoftness = 0.9;
const tronRunnerSuitTexture = makeTronRunnerSuitTexture();
const tronRunnerSuitEmissiveTexture = makeTronRunnerSuitEmissiveTexture();
const tronRunnerSuitLedMaskTexture = makeTronRunnerSuitLedMaskTexture();
...
const TRON_RUNNER_CROWD_COLOR_PLAN = [
  'current',
  'orange',
  'current',
  'red',
...
const TRON_RUNNER_CROWD_COLOR_PRESETS = {
  current: {
    label: 'current',
    name: 'current cyan',
...
function tronRunnerBeatPulseValue(audioTime = tronRunnerSoundtrackTimeSeconds()) {
  ...
  const now = performance.now();
  const sampleAgeMs = Number.isFinite(labEqualizerLastSampleAt) ? now - labEqualizerLastSampleAt : Infinity;
  const analyserReady = Boolean(labEqualizerAnalyser && labEqualizerFrequencyData && labEqualizerState.sampleCount > 0);
...
function applyTronRunnerVisualControls() {
  ...
  if (tronRunnerParts.groundShadow) {
    ...
    if (Math.abs(shadowSoftness - tronRunnerShadowTextureSoftness) > 0.001) {
      ...
      tronRunnerShadowTexture = makeTronRunnerShadowTexture(shadowSoftness);
...
function tronRunnerCrowdPointClearOfRecord(record, worldX, worldZ, padding = TRON_RUNNER_CROWD_BUILDING_GUARD) {
  ...
  return !resolveTronRunnerRoundedCollider(point, record.collider, padding);
}

function tronRunnerCrowdPadPointSafe(record, localX, localZ, polygon, padding = TRON_RUNNER_CROWD_BUILDING_GUARD) {
  const pad = record?.basePad;
  if (!pad?.border || !polygon?.length) return false;
  if (!pointInBasePadPolygon(localX, localZ, polygon)) return false;
  return tronRunnerCrowdPointClearOfRecord(
    record,
    pad.border.position.x + localX,
...
function tronRunnerCrowdLoopRouteForRecord(record, index) {
  ...
  const guard = TRON_RUNNER_CROWD_BUILDING_GUARD + TRON_RUNNER_CROWD_COLLISION_RADIUS * 1.35;
  ...
  const y = (pad.innerTopY ?? pad.topY ?? roadTileTopY()) + TRON_RUNNER_CROWD_GROUND_OFFSET;
  if (TRON_RUNNER_CROWD_LOOP_FRONT_ONLY) {
    const points = [
...
function tronRunnerCrowdRoadFacingRoutePoint(record, offsetZ = 0) {
  ...
  const roadDir = tronRunnerCrowdRecordRoadDir(record);
  const edgeInset = Math.max(TRON_RUNNER_CROWD_SIDEWALK_LANE_EDGE_INSET + 0.8, 2);
  ...
  if (!tronRunnerCrowdPadPointSafe(record, localX, localZ, polygon, TRON_RUNNER_CROWD_BUILDING_GUARD)) return null;
  const y = (pad.innerTopY ?? pad.topY ?? roadTileTopY()) + TRON_RUNNER_CROWD_GROUND_OFFSET;
...
function tronRunnerCrowdSideStreetRouteForPair(pair, index) {
  ...
  const offsetStep = Math.min(GRID_BLOCK * 0.22, SIDE_BASE * 0.16);
  const offsetZ = ((index % 3) - 1) * offsetStep;
  const left = tronRunnerCrowdRoadFacingRoutePoint(pair.left, offsetZ);
  const right = tronRunnerCrowdRoadFacingRoutePoint(pair.right, offsetZ);
...
function tronRunnerCrowdDetectedSideStreetLanes(records) {
  ...
  for (const [sideSign, sideRecords] of bySide) {
    ...
    for (let i = 0; i < sorted.length - 1; i += 1) {
      ...
      if (clearZ < TRON_RUNNER_CROWD_COLLISION_RADIUS * 5) continue;
      const z = (az + bz) * 0.5;
      const sideWidth = sideBuildingVisualWidth();
      const inset = Math.max(GRID_BLOCK * 0.7, TRON_RUNNER_CROWD_COLLISION_RADIUS * 2.8);
      const innerX = sideSign * (roadHalf() + inset);
      const outerX = sideSign * (roadHalf() + streetEdgeWidth + sideWidth - inset);
      if (Math.abs(outerX - innerX) < GRID_BLOCK * 2.2) continue;
      lanes.push({
        sideSign,
        z,
        innerX,
  ...
  const sortedLanes = lanes.sort((a, b) => {
    if (Math.abs(a.z - b.z) > GRID_BLOCK * 0.5) return b.z - a.z;
    return a.sideSign - b.sideSign;
  });
  let streetIndex = 0;
  let lastZ = null;
  for (const lane of sortedLanes) {
    if (lastZ === null || Math.abs(lane.z - lastZ) > GRID_BLOCK * 0.5) {
      streetIndex += 1;
      lastZ = lane.z;
    }
...
function tronRunnerCrowdStartPlayerRoute(index) {
  ...
  const laneXs = [-0.56, -0.28, 0, 0.28, 0.56].map((ratio) => (
    THREE.MathUtils.clamp(
      ratio * roadHalf(),
      limits.minX + GRID_BLOCK * 1.5,
      limits.maxX - GRID_BLOCK * 1.5
    )
  ));
  const slot = index % TRON_RUNNER_CROWD_START_CLUSTER_COUNT;
  const x = laneXs[slot] ?? 0;
  const zCenter = THREE.MathUtils.clamp(
    anchor.z - GRID_BLOCK * 1.5,
    limits.minZ + GRID_BLOCK * 4,
    limits.maxZ - GRID_BLOCK * 4
  );
  const zOffset = (slot - (TRON_RUNNER_CROWD_START_CLUSTER_COUNT - 1) * 0.5) * GRID_BLOCK * 0.55;
  const zA = THREE.MathUtils.clamp(zCenter + zOffset + GRID_BLOCK * 2.2, limits.minZ + GRID_BLOCK * 3, limits.maxZ - GRID_BLOCK * 3);
  const zB = THREE.MathUtils.clamp(zCenter + zOffset - GRID_BLOCK * 3.6, limits.minZ + GRID_BLOCK * 3, limits.maxZ - GRID_BLOCK * 3);
  const y = roadTileTopY() + TRON_RUNNER_CROWD_GROUND_OFFSET;
  return {
...
function tronRunnerCrowdRouteForRecord(record, index) {
  ...
  const colliderX = record.collider.x - pad.border.position.x;
  const colliderZ = record.collider.z - pad.border.position.z;
  const routeEndInset = Math.min(
    TRON_RUNNER_CROWD_SIDEWALK_ROUTE_END_INSET,
    Math.max(1, (maxZ - minZ) * 0.34)
  );
  const routeNear = minZ + routeEndInset;
  const routeFar = maxZ - routeEndInset;
  if (routeFar - routeNear < TRON_RUNNER_CROWD_REACH_RADIUS * 3) return null;

  const roadDir = (record.sign || 1) < 0 ? 1 : -1;
  const roadEdgeX = roadDir > 0
    ? maxX - TRON_RUNNER_CROWD_SIDEWALK_LANE_EDGE_INSET
    : minX + TRON_RUNNER_CROWD_SIDEWALK_LANE_EDGE_INSET;
  const guardX = colliderX + roadDir * (record.collider.hw + TRON_RUNNER_CROWD_BUILDING_GUARD);
  let routeLaneX = null;
  for (let step = 0; step <= 36; step += 1) {
    ...
    if (
      pointInBasePadPolygon(candidateX, routeNear, polygon)
      && pointInBasePadPolygon(candidateX, routeFar, polygon)
    ) {
...
function tronRunnerCrowdBuildRoute(index) {
  const records = tronRunnerCrowdCandidateRecords();
  const startCluster = index < TRON_RUNNER_CROWD_START_CLUSTER_COUNT;
  if (startCluster) {
    const route = tronRunnerCrowdStartPlayerRoute(index);
    route.spawnSlot = index;
    route.activeRouteRecords = TRON_RUNNER_CROWD_START_CLUSTER_COUNT;
    return route;
  }
  if (!records.length) return null;
  const routeIndex = index - TRON_RUNNER_CROWD_START_CLUSTER_COUNT;
  ...
  const style = tronRunnerCrowdRouteStyleForIndex(routeIndex);
  const sideStreetPairs = style === 'side-street-crossing'
    ? tronRunnerCrowdSideStreetPairs(routeRecords)
    : [];
  const sideStreetPair = sideStreetPairs.length
    ? sideStreetPairs[routeIndex % sideStreetPairs.length]
    : null;
  const sideStreetLanes = style === 'side-street-lateral'
    ? tronRunnerCrowdDetectedSideStreetLanes(routeRecords)
    : [];
  const sideStreetLateralOrdinal = style === 'side-street-lateral'
    ? tronRunnerCrowdRouteStyleOrdinal(routeIndex, style)
    : 0;
  const sideStreetAssignment = style === 'side-street-lateral'
    ? tronRunnerCrowdSideStreetLaneAssignment(sideStreetLanes, sideStreetLateralOrdinal)
    : null;
  const sideStreetLane = sideStreetAssignment?.lane ?? null;
  const route = (
    style === 'single-building-loop'
      ? tronRunnerCrowdLoopRouteForRecord(record, routeIndex)
      : style === 'side-street-lateral'
        ? tronRunnerCrowdSideStreetLateralRoute(sideStreetLane, routeIndex, sideStreetLateralOrdinal, sideStreetAssignment)
      : style === 'side-street-crossing'
        ? tronRunnerCrowdSideStreetRouteForPair(sideStreetPair, routeIndex)
        : null
  ) || tronRunnerCrowdRouteForRecord(record, routeIndex);
  if (route) {
...
function setTronRunnerCrowdFrameDistance(member, distance) {
  ...
  member.cullingDistanceFrame = tronRunnerCrowdRuntimeStats.frame;
}
...
function tronRunnerCrowdDistanceToCamera(member) {
  if (
    ...
    tronRunnerCrowdRuntimeStats.distanceReuses += 1;
    return member.cullingDistance;
  }
  const distance = camera.position.distanceTo(member.group.position);
  tronRunnerCrowdRuntimeStats.distanceCalculations += 1;
  setTronRunnerCrowdFrameDistance(member, distance);
  return distance;
}
...
function tronRunnerCrowdLodStride(member) {
  ...
  member.lodDistance = distance;
  if (distance <= TRON_RUNNER_CROWD_LOD_NEAR_DISTANCE) return 1;
  if (distance <= TRON_RUNNER_CROWD_LOD_MID_DISTANCE) return 2;
  return 4;
}
...
function updateTronRunnerCrowdCulling() {
  ...
  tronRunnerCrowdRuntimeStats.cullingHiddenCount = 0;
  tronRunnerCrowdRuntimeStats.cullingDistanceHiddenCount = 0;
  tronRunnerCrowdRuntimeStats.cullingFrustumHiddenCount = 0;
  tronRunnerCrowdRuntimeStats.cullingMinDistance = 0;
  tronRunnerCrowdRuntimeStats.cullingMaxDistance = 0;
  tronRunnerCrowdRuntimeStats.distanceCalculations = 0;
  tronRunnerCrowdRuntimeStats.distanceReuses = 0;
  if (!tronRunnerCrowdGroup.visible || !tronRunnerCrowd.length) return;

  if (!TRON_RUNNER_CROWD_CULLING_ENABLED) {
    tronRunnerCrowdRuntimeStats.cullingVisibleCount = tronRunnerCrowd.length;
    for (const member of tronRunnerCrowd) {
      member.cullingDistance = camera.position.distanceTo(member.group.position);
      tronRunnerCrowdRuntimeStats.distanceCalculations += 1;
      setTronRunnerCrowdFrameDistance(member, member.cullingDistance);
  ...
  camera.updateMatrixWorld();
  tronRunnerCrowdCullMatrix.multiplyMatrices(camera.projectionMatrix, camera.matrixWorldInverse);
  tronRunnerCrowdCullFrustum.setFromProjectionMatrix(tronRunnerCrowdCullMatrix);
  ...
  for (const member of tronRunnerCrowd) {
    ...
    tronRunnerCrowdCullSphere.center.copy(member.group.position);
    tronRunnerCrowdCullSphere.center.y += TRON_RUNNER_TARGET_HEIGHT * member.group.scale.y * 0.5;
    tronRunnerCrowdCullSphere.radius = TRON_RUNNER_CROWD_CULL_RADIUS;
    const inDistance = distance <= TRON_RUNNER_CROWD_CULL_DISTANCE;
    const inFrustum = tronRunnerCrowdCullFrustum.intersectsSphere(tronRunnerCrowdCullSphere);
    ...
    if (visible) {
      tronRunnerCrowdRuntimeStats.cullingVisibleCount += 1;
    } else {
      tronRunnerCrowdRuntimeStats.cullingHiddenCount += 1;
      if (!inDistance) tronRunnerCrowdRuntimeStats.cullingDistanceHiddenCount += 1;
      if (inDistance && !inFrustum) tronRunnerCrowdRuntimeStats.cullingFrustumHiddenCount += 1;
    }
  ...
  tronRunnerCrowdRuntimeStats.cullingMinDistance = Number.isFinite(minDistance) ? minDistance : 0;
  tronRunnerCrowdRuntimeStats.cullingMaxDistance = maxDistance;
}

function tronRunnerCrowdWalkCycleOffset(index) {
...
function tronRunnerCrowdRoadFacingStart(route, index, fallback) {
  ...
  if (pad?.border && route.polygon?.length) {
    for (let step = 0; step <= 36; step += 1) {
      ...
      if (!pointInBasePadPolygon(localX, localZ, route.polygon)) break;
      roadSideX = candidateX;
    }
    roadSideX += sign * TRON_RUNNER_CROWD_COLLISION_RADIUS * 1.8;
  }
  return {
    placement: {
      x: roadSideX,
...
function tronRunnerCrowdPointInsideRoute(member, x, z) {
  ...
  const localZ = z - pad.border.position.z;
  return pointInBasePadPolygon(localX, localZ, polygon);
}
...
function tronRunnerCrowdColliderLabel(record) {
...
function tronRunnerCrowdBuildingCollisionDiagnostic(member) {
  ...
  for (const record of tronRunnerCrowdColliderRecords()) {
    ...
    const beforeX = point.x;
    const beforeZ = point.z;
    if (!resolveTronRunnerRoundedCollider(point, record.collider, TRON_RUNNER_CROWD_BUILDING_GUARD)) continue;
    const nextCorrection = Math.hypot(point.x - beforeX, point.z - beforeZ);
    colliding = true;
    if (nextCorrection >= correction) {
      correction = nextCorrection;
      label = tronRunnerCrowdColliderLabel(record);
    }
  }
  return {
    colliding,
...
function tronRunnerPhaseCrossed(previous, current, target) {
...
function tronRunnerWalkCycleFootstep(movedDistance) {
  ...
  if (phase !== null) {
    const previous = tronRunnerAutonomy.lastWalkCyclePhase;
    tronRunnerAutonomy.lastWalkCyclePhase = phase;
    if (movedDistance <= 0.001 || previous === null) {
      return { triggered: false, syncSource: 'walk-cycle', phase };
    }
    for (const contact of TRON_RUNNER_FOOTSTEP_CONTACTS) {
      if (tronRunnerPhaseCrossed(previous, phase, contact.phase)) {
        return { triggered: true, side: contact.side, syncSource: 'walk-cycle', phase };
      }
    }
    return { triggered: false, syncSource: 'walk-cycle', phase };
  }

  tronRunnerAutonomy.footstepPhase += movedDistance / TRON_RUNNER_FREE_ROAM_STRIDE_LENGTH * Math.PI;
  const nextStepIndex = Math.floor(tronRunnerAutonomy.footstepPhase / Math.PI);
  if (movedDistance <= 0.001 || nextStepIndex === tronRunnerAutonomy.lastFootstepIndex) {
    return { triggered: false, syncSource: 'distance-phase' };
  }
  tronRunnerAutonomy.lastFootstepIndex = nextStepIndex;
  tronRunnerAutonomy.footstepSide = 1 - tronRunnerAutonomy.footstepSide;
  return {
    triggered: true,
    side: tronRunnerAutonomy.footstepSide ? 'right' : 'left',
...
function resolveTronRunnerRoundedCollider(point, collider, padding) {
...
function fitTronRunnerModel(model) {
  ...
  model.scale.setScalar(TRON_RUNNER_TARGET_HEIGHT / Math.max(initialSize.y, 0.001));
  model.scale.x *= 0.74;
  model.scale.z *= 0.70;
...
function makeTronRunnerActionSet(gltf, mixer) {
  const actions = {};
...
function switchTronRunnerAction(next, previous) {
  ...
  next.reset();
  next.enabled = true;
  next.setEffectiveTimeScale(tronRunnerEffectiveAnimationSpeed());
...
async function loadTronRunner() {
  ...
  tronRunnerAutonomy.initialized = false;
  tronRunnerAutonomy.waypointIndex = 0;
  tronRunnerAutonomy.distanceWalked = 0;
  tronRunnerAutonomy.collisionCount = 0;
  tronRunnerAutonomy.lastCollision = false;
  tronRunnerAutonomy.footstepPhase = 0;
  tronRunnerAutonomy.lastFootstepIndex = -1;
  tronRunnerAutonomy.lastFootstepPlayedAt = 0;
  tronRunnerAutonomy.lastWalkCyclePhase = null;
  tronRunnerAutonomy.lastFootstepSurface = '';
  tronRunnerAutonomy.lastFootstepSample = '';
  tronRunnerAutonomy.lastFootstepPlayed = false;
  tronRunnerAutonomy.lastFootstepBus = FOOTSTEP_NPC_SPATIAL_BUS;
  tronRunnerAutonomy.lastFootstepGain = 0;
  tronRunnerAutonomy.lastFootstepDistance = 0;
  tronRunnerAutonomy.lastFootstepDistanceGain = 1;
  tronRunnerAutonomy.lastFootstepPlaybackRate = 0;
  tronRunnerAutonomy.lastFootstepPan = 0;
  tronRunnerAutonomy.lastFootstepSyncSource = 'walk-cycle';
...
const DEFAULT_PRODUCTION_PANEL = 'bloom-fx';
const PRODUCTION_CONTROL_PANELS = new Set([
  'movement',
  'character',
  'surface-reflections',
  'bloom-fx',
...
const PRODUCTION_LIVE_CONTROL_SELECTOR = Array.from(PRODUCTION_CONTROL_PANELS).flatMap((panel) => [
...
const SKY_FAST_CONTROL_IDS = new Set([
  'sky-choice',
  'sky-quality',
  'sky-brightness',
  'sky-hue',
...
const POST_FAST_CONTROL_IDS = new Set([
  'performance-mode',
  'render-resolution',
  'aa-mode',
  'bloom-enabled',
...
const WIREFRAME_FAST_CONTROL_IDS = new Set([
  'wireframe-enabled',
  'wireframe-delay',
  'wireframe-fade',
  'wireframe-density',
...
const AUDIO_FX_FAST_CONTROL_IDS = new Set([
  'music-fx-enabled',
  'music-fx-mix',
  'music-fx-crusher',
  'music-fx-bit-depth',
...
const MOVEMENT_FAST_CONTROL_IDS = new Set([
  'collision-padding',
...
const CHARACTER_FAST_CONTROL_IDS = new Set([
  'runner-body-light',
  'runner-line-light',
  'runner-key-light',
  'runner-rim-light',
...
const LIGHT_FAST_CONTROL_IDS = new Set(['ambient-light', 'key-light', 'exposure']);
const HEX_RUNTIME_FAST_CONTROL_IDS = new Set([
  'hex-offset',
  'hex-radius',
  'hex-drop-delay',
  'hex-drop-speed',
...
const ROAD_MATERIAL_FAST_CONTROL_IDS = new Set([
  'road-normal',
  'road-light',
  'road-reflect',
...
const BUILDING_MATERIAL_FAST_CONTROL_IDS = new Set([
...
const BASE_PAD_MATERIAL_FAST_CONTROL_IDS = new Set([
...
const BOUNDARY_ERROR_FAST_CONTROL_IDS = new Set([
